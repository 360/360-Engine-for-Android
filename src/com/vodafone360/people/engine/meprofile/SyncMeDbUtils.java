...
public class SyncMeDbUtils {
    ...
    private static String processMyContactDetailsChanges(final DatabaseHelper dbHelper,
        ...
        for (ContactDetail srcDetail : profileChanges.details) {
            boolean found = false;
            for (int i = 0; i < currentMeProfile.details.size(); i++) {
                ContactDetail destDetail = currentMeProfile.details.get(i);
                if (DatabaseHelper.doDetailsMatch(srcDetail, destDetail)) {
                    found = true;
                    if (srcDetail.deleted != null && srcDetail.deleted.booleanValue()) {
                        deletedDetailList.add(destDetail);
                    } else if (DatabaseHelper.hasDetailChanged(srcDetail, destDetail)) {
                        srcDetail.localDetailID = destDetail.localDetailID;
                        srcDetail.localContactID = destDetail.localContactID;
                        srcDetail.nativeContactId = destDetail.nativeContactId;
                        srcDetail.nativeDetailId = destDetail.nativeDetailId;
                        modifiedDetailList.add(srcDetail);

                        if (srcDetail.key == DetailKeys.PHOTO) {
            ...
            if (!found) {
                srcDetail.localContactID = currentMeProfile.localContactID;
                srcDetail.nativeContactId = currentMeProfile.nativeContactId;
                addedDetailList.add(srcDetail);
            }
        }
    ...
    public static void updateMeProfileDbDetailIds(final DatabaseHelper dbHelper,
        ...
        for (ContactDetail srcDetail : result.mUserProfile.details) {
            ...
            if (srcDetail.unique_id != null && !srcDetail.unique_id.equals(destDetail.unique_id)) {
                destDetail.unique_id = srcDetail.unique_id;
                dbHelper
                        .modifyContactDetailServerId(destDetail.localDetailID, destDetail.unique_id);
            }
        }
    }
                    ...
                    if (oldStatus.key == ContactDetail.DetailKeys.PRESENCE_TEXT) {
                        for (ContactDetail newStatus : result.mUserProfile.details) {
                            if (newStatus.key == ContactDetail.DetailKeys.PRESENCE_TEXT) {
                                if (newStatus.unique_id != null
                                        && (!newStatus.unique_id.equals(oldStatus.unique_id))) {
                                    oldStatus.unique_id = newStatus.unique_id;
                                    dbHelper.modifyContactDetailServerId(oldStatus.localDetailID,
                                            oldStatus.unique_id);
                                    break;
                                }
                            }
    ...
    private static void populateWithModifiedDetails(final DatabaseHelper dbHelper,
        ...
        for (ContactDetail detail : meProfile.details) {
            ...
            if (avatarChanged && detail.key == ContactDetail.DetailKeys.PHOTO) {
                avatarUpdated = populatePhotoDetail(dbHelper, meProfile, detail);
                if (avatarUpdated) {
                    updates.add(detail);
                    // LogUtils.logV("meProfile.details: put new photo");
                }
        ...
        // if this detail has never existed before
        if (avatarChanged && !avatarUpdated) {
            // fill in the photo
            ContactDetail photoDetail = new ContactDetail();
            photoDetail.key = ContactDetail.DetailKeys.PHOTO;
            if (populatePhotoDetail(dbHelper, meProfile, photoDetail)) {
                updates.add(photoDetail);
                // LogUtils.logV("meProfile.details: put new photo");
            }
        }
    }
     ...
     * @return boolean - TRUE if the details was read successfully.
     */
    private static boolean populatePhotoDetail(final DatabaseHelper dbHelper, final Contact meProfile,
