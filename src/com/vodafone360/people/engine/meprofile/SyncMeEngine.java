...
public class SyncMeEngine extends BaseEngine {
     ...
     */
    public SyncMeEngine(final IEngineEventCallback eventCallback, DatabaseHelper db) {
        super(eventCallback);
    ...
    private void processMeProfileThumbnailResponse(final Response resp) {
        ...
        if (status == ServiceStatus.SUCCESS) {
            ...
            status = BaseEngine
                    .genericHandleResponseType("ExternalResponseObject", resp.mDataTypes);
            if (status != ServiceStatus.SUCCESS) {
    ...
    private void processGetMyChangesResponse(final Response resp) {
        ...
        ServiceStatus status = BaseEngine.genericHandleResponseType(CONTACT_CHANGES,
                resp.mDataTypes);
        setSyncStatus(new SyncStatus(PROGRESS_50, "Me Profile", Task.SYNC_ME_PROFILE));
        if (status == ServiceStatus.SUCCESS) {
    ...
    public void addGetMeProfileContactFirstTimeRequest() {
        ...
        setSyncStatus(new SyncStatus(0, "", SyncStatus.Task.SYNC_ME_PROFILE));
     ...
     */
    public void setSyncStatus(final SyncStatus syncStatus) throws InvalidParameterException {
        if (syncStatus == null) {
