...
public class ChatDbUtils {
     ...
     */
    /*
     * public static String parseUserName(String userId) { if (userId != null) {
     * int columnsIndex = userId.indexOf(COLUMNS); if (columnsIndex >-1) {
     * return userId.substring(columnsIndex + COLUMNS.length()); } else { return
     * userId; } } return null; }
     */
    ...
    protected static void updateUnsentChatMessage(DatabaseHelper dbHelper) {
        TimelineSummaryItem msg = ActivitiesTable.getNewestOutgoingChatMessage(dbHelper
                .getReadableDatabase());

        // retry with new conversation id
        Long localId = msg.mLocalContactId;
        if (localId != null && (localId != -1)) {
            SocialNetwork network = SocialNetwork.getValue(msg.mContactNetwork);
            if (network != null) {
                ActivitiesTable.updateMessageAsUnsent(localId, msg.mContactNetwork, dbHelper
                        .getWritableDatabase());
                ConversationsTable.removeConversation(localId, network.ordinal(), dbHelper
                        .getWritableDatabase());
            }
        }
    }
