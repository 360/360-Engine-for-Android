...
/**
 * The NativeImporter class is responsible for importing contacts from the native address book to the people database.
 * 
 * To do so, it compares the native address book and the people database to find new changes and update the people
 * database accordingly.
 * 
 * Usage:
 *  - instantiate the class
 *  - call the tick() method until isDone() returns true
 *  - current progress can be obtained via getPosition() / getCount()
 *  - check the getResult() to get a status (OK, KO...)
 */
public class NativeImporter {
    ...
    /**
     * Internal state representing the task to perform: iterates through the list of ids to find differences
     * (i.e. added contact, modified contact, deleted contact).
     */
    private final static int STATE_ITERATE_THROUGH_IDS = 1;
    
    /**
     * Internal state representing the task to perform: process the list of deleted contacts
     * (i.e. delete the contacts from the people database).
     * 
     ...
     * 
     * Note: if null, will import from the platform default account.
     */
     ...
     * @param firstTimeImport true if we import from native for the first time, false if not
     */
     ...
     * 
     * Note: the method will block for some time to process a certain amount of contact then will
    ...
    private void getIdsLists() {
        ...
        // Get a list of native ids for the contacts we have in the People database
    ...
    private void iterateThroughNativeIds() {
        ...
        final int limit = Math.min(mNativeContactsIds.length, mCurrentNativeId + MAX_CONTACTS_OPERATION_COUNT);
        
        // TODO: remove the deleted state / queuing to deleted ids array and loop with while (mProcessedIds < limit)
        while (mCurrentNativeId < limit) {
            ...
            if (mPeopleNativeContactsIds == null) {
                ...
                while ((mCurrentPeopleId < mPeopleNativeContactsIds.length)
                    ...
                    LogUtils.logD("NativeImporter.iterateThroughNativeIds(): found a contact to delete");
                ...
                if (mCurrentPeopleId == mPeopleNativeContactsIds.length
                 || mPeopleNativeContactsIds[mCurrentPeopleId] > mNativeContactsIds[mCurrentNativeId]) {
                    ...
                    LogUtils.logD("NativeImporter.iterateThroughNativeIds(): check existing contact");
    ...
    private void processDeleted() {
        ...
        final int limit = Math.min(mDeletedIds.size(), mCurrentDeletedId + MAX_CONTACTS_OPERATION_COUNT);
        
        while (mCurrentDeletedId < limit) {
            
            // we now delete the contacts on people client side
            // on the 2.X platform, the contact deletion has to be synced back to native once completed because the 
            // contact is still there on native, just marked as deleted and waiting for its explicit removal 
            mPeopleContactsApi.deleteNativeContact(mDeletedIds.get(mCurrentDeletedId++), VersionUtils.is2XPlatform());
            mProcessedIds++;
        }
    ...
    private void addNewContact(long nativeId) {
        ...
        if (contactChanges != null) {
            ...
            if (mIsFirstImportOn2X) {
                ...
                // Override the nativeContactId with an invalid id if we are on 2.X
                // and we are doing a first time import because the native id does not correspond 
                // to the id from the 360 People account where we will export.
                removeNativeIds(contactChanges);
            } else {
                
                // Force a nativeDetailId to details that have none so that the comparison
            ...
            if (!mPeopleContactsApi.addNativeContact(contactChanges)) {
                ...
                LogUtils.logE("NativeImporter.addNewContact() - failed to import native contact id=" + nativeId);
            }
        }
    }
     ...
     * 
     * This class compares ContactChange and tells which one is greater depending on the key
     * and then the native detail id.
     */
     ...
     * 
     * Note: the method will rearrange the provided array.
     * 
     ...
     * 
     * The delta are the changes to apply to the master ContactChange array to
     * make it similar to the new changes ContactChange array.
     * (i.e. delete details, add details or modify details)
     * 
     * NOTE: to help the GC, some provided ContactChange may be modified and returned by the method
     *       instead of creating new ones.
     * 
     * @param masterChanges the master array of ContactChange (i.e. the original one)
     * @param newChanges the new ContactChange array (i.e. contains the new version of a contact)
     * @return null if there are no changes or the contact is being deleted, an array for ContactChange to apply 
     *         to the master contact if differences where found 
     */
    private ContactChange[] computeDelta(ContactChange[] masterChanges, ContactChange[] newChanges) {
        ...
        // set the native contact id to details that don't have a native detail id for the comparison as
        ...
        final ContactChange[] deltaChanges = new ContactChange[masterChanges.length + newChanges.length];
        int deltaIndex = 0;
        int masterIndex = 0;
        int newIndex = 0;
        ...
        while (newIndex < newChanges.length) {
            ...
            if ((masterIndex < masterChanges.length) &&
                ...
                // similar key and id, check for differences at value level and flags
