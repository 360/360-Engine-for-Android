...
public class NativeContactsApi1 extends NativeContactsApi {
     ...
     */
	private final static String[] CONTACTID_PROJECTION = { People._ID };
	/**
	 * Convenience Projection for the NATE and NOTES from the People table
	 */
	private final static String[] PEOPLE_PROJECTION = { People.NAME, People.NOTES };
    ...
    public void registerObserver(ContactsObserver observer) {
        ...
        if (mContactsObserver != null) {
            throw new RuntimeException("NativeContactsApi1.registerObserver(): current implementation only supports one observer" +
    ...
    public long[] getContactIds(Account account) {    
        if(account != null) {
            ...
            LogUtils.logE(
                    "NativeContactsApi1.getContactIds() Unexpected non-null Account(\""+
                    account.toString()+"\"");
            return null;
        }
        
        long[] ids = null;
        final Cursor cursor = mCr.query(People.CONTENT_URI, CONTACTID_PROJECTION, null, null, People._ID);
        try {
    ...
    public ContactChange[] addContact(Account account, ContactChange[] ccList) {
        ...
        /*
         * These details need special treatment:
         * Name and Note (written to people table);
         * Organization/Title (written as one detail in NAB).
         */
        ...
        if(uri != null) {
            // Change List to hold resulting changes for the add contact (including +1 for contact id)
    ...
    public ContactChange[] updateContact(ContactChange[] ccList) {
        if(ccList == null || ccList.length == 0) {
            LogUtils.logW(
                    "NativeContactsApi1.updateContact() nothing to update - empty ccList!");
            return null;
        }
        ...
        for(int i = 0; i < ccListSize; i++) {
            ...
            switch(cc.getType()) {
                case ContactChange.TYPE_ADD_DETAIL:
                    ...
                    if(key != ContactChange.KEY_VCARD_NAME && 
                            key != ContactChange.KEY_VCARD_NOTE) {
                        final long nabDetailId = 
                            insertDetail(cc, nabContactId);
                        ...
                        // Name and Note are not inserted but updated because they are always there!
                        updateDetail(cc);
                    }
        ...
        if(orgDetailId != ContactChange.INVALID_ID) {
            if(mMarkedOrganizationIndex >= 0 && 
                ...
                idCcList[mMarkedOrganizationIndex] = 
                    ContactChange.createIdsChange(
    ...
    /**
     * Reads Phone Number details from a Contact into the provided {@link ContactChange} List
     * @param ccList {@link ContactChange} list to add details into
     * @param nabContactId ID of the NAB Contact
     */
    private void readContactPhoneNumbers(List<ContactChange> ccList, 
            long nabContactId) {
    ...
    /**
     * Reads one Phone Number Detail from the supplied cursor into a {@link ContactChange}
    ...
    /**
     * Reads Contact Method details from a Contact into the provided {@link ContactChange} List
     * @param ccList {@link ContactChange} list to add details into
     * @param nabContactId ID of the NAB Contact
     */
    ...
    /**
     * Reads one Contact Method Detail from the supplied cursor into a {@link ContactChange}
     * @param cursor Cursor to read from
        	...
        	if(kind == CONTACT_METHODS_KIND_EMAIL) {
        	    ...
        	    if(!TextUtils.isEmpty(value)) {
                    cc = new ContactChange(
                            ...
                            VCardHelper.makePostalAddress(parseRawAddress(value)), 
                            flags);
                    cc.setNabDetailId(nabDetailId);
        	    }
        	}
    ...
    /**
     * Reads Organization details from a Contact into the provided {@link ContactChange} List
     * @param ccList {@link ContactChange} list to add details into
     * @param nabContactId ID of the NAB Contact
     */
    private void readContactOrganizations(List<ContactChange> ccList, 
            long nabContactId) {       
        final Uri contactUri = ContentUris.withAppendedId(People.CONTENT_URI, nabContactId);
    ...
    /**
     * Reads one Organization Detail from the supplied cursor into a {@link ContactChange}
    ...
    private void readContactOrganization(Cursor cursor, List<ContactChange> ccList) {
		...
		if(!mHaveReadOrganization) {
        	...
        	if(!TextUtils.isEmpty(company)) {
    		    // Escaping the value (no need to involve the VCardHelper just for the company)
    ...
    /**
     * "Pre-processing" of {@link ContactChange} list before it can be added to NAB.
    ...
    private void preprocessContactToAdd(ContactChange[] ccList) {
        ...
        for(int i = 0; i < ccListSize; i++) {
            ...
            if(cc != null) {    
                if(!nameFound && 
                        cc.getKey() == ContactChange.KEY_VCARD_NAME) {
                ...
                if(!noteFound && 
                        cc.getKey() == ContactChange.KEY_VCARD_NOTE) {
                    mValues.put(People.NOTES, cc.getValue());
                    noteFound = true;
                }
                
                if(mMarkedOrganizationIndex < 0 && 
                        cc.getKey() == ContactChange.KEY_VCARD_ORG) {
                    mMarkedOrganizationIndex = i;
                }
                
                if(mMarkedTitleIndex < 0 && 
                        cc.getKey() == ContactChange.KEY_VCARD_TITLE) {
                    mMarkedTitleIndex = i;
                }
            }
        }        
    ...
    /**
     * Writes new details from a provided {@link ContactChange} list into the NAB.
     * The resulting detail IDs are put in another provided {@link ContactChange} list.
     * @param ccList {@link ContactChange} list to write from
     * @param idChangeList {@link ContatChange} list where IDs for the written details are put
     * @param nabContactId ID of the NAB Contact
     */
    private void writeDetails(ContactChange[] ccList, 
            ContactChange[] idChangeList, long nabContactId) {
        final int ccListSize = ccList.length;
        for(int i = 0; i < ccListSize; i++) {
            final ContactChange cc = ccList[i]; 
            if(cc != null) {
                ...
                if(nabDetailId != ContactChange.INVALID_ID) {
                    // The +1 assumes prior addition of the contact id at position 0
                    idChangeList[i+1] = ContactChange.createIdsChange(
                            cc, ContactChange.TYPE_UPDATE_NAB_DETAIL_ID);
    ...
    private void putPhoneValues(ContactChange cc, long nabContactId) {
         ...
         * custom labels and in case we replace
         * from a custom label to home or private 
         * type without setting label to null, 
         * mCr.update() will throw a SQL constraint exception.
         */
    ...
    private void putContactMethodValues(ContactChange cc, int kind, long nabContactId) {
        ...
        if(kind == CONTACT_METHODS_KIND_EMAIL) {
            ...
            // Must be Address, once again need to use VCardHelper to extract address
     ...
     * @param idChangeList {@link ContatChange} list where IDs for the written organization are put
     * @param nabContactId ID of the NAB Contact
     */
    private void processOrganization(ContactChange[] ccList, 
        ...
        if(organizationId != ContactChange.INVALID_ID) {
            if(mMarkedOrganizationIndex >= 0) {
                idChangeList[mMarkedOrganizationIndex + 1] = 
                    ContactChange.createIdsChange(
            ...
            if(mMarkedTitleIndex >= 0) {
                idChangeList[mMarkedTitleIndex + 1] = 
                    ContactChange.createIdsChange(
     ...
     * @param ccList {@link ContactChange} list where Organization and Title are found
     * @param nabContactId ID of the NAB Contact
    ...
    private long writeOrganization(ContactChange[] ccList, long nabContactId) {
        ...
        if(mMarkedOrganizationIndex >= 0) {
            ...
            final Organisation organization = 
                VCardHelper.getOrg(cc.getValue());
     ...
     * @return In the Organization insertion case this should contain the new ID and in the update case should contain the existing ID
     */
    private long updateOrganization(ContactChange[] ccList, long nabContactId) {        
        ...
        final Uri organizationUri = Uri.withAppendedPath(
                ContentUris.withAppendedId(People.CONTENT_URI, nabContactId), 
                Contacts.Organizations.CONTENT_DIRECTORY);
        ...
        if(company != null || title != null) {
            mValues.clear();
            /* Forcing the label field to be null 
             * because we don't support custom
             * labels and in case we replace
             * from a custom label to home or private type without 
             * setting label to null, mCr.update() will throw
             * a SQL constraint exception.
             */ 
    ...
    /**
     * Maps a method type from the native value into the {@link ContactChange} flags
    ...
    /**
     * Utility method used to parse a raw display Address 
     * into a {@link VCardHelper.PostalAddress} object.
