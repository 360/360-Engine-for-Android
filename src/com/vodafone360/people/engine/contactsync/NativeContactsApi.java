...
public abstract class NativeContactsApi { 
     ...
     */
    protected static final String PEOPLE_ACCOUNT_TYPE = "com.vodafone360.people.android.account";
     ...
     * The instance can retrieved by calling getInstance().
     * * The instance can be destroyed by calling destroyInstance()
    ...
    public static void createInstance(Context context) {
        ...
        try {
            Class<? extends NativeContactsApi> clazz =
                    Class.forName(NativeContactsApi.class.getPackage().getName() + "." + className)
                            .asSubclass(NativeContactsApi.class);
     ...
     * The instance can be recreated by calling createInstance()
    ...
    /**
     * This Account class represents an available account on the device
     * where the native synchronization can be performed.
     */
    public static class Account {
        ...
        public boolean isPeopleAccount() {
            return TextUtils.equals(mType, PEOPLE_ACCOUNT_TYPE);
        }
     ...
     * This method effectively acts as a replacement for the constructor 
     * because of the use of reflection. 
     */
     ...
     * 
     * Note: the method only supports one observer at a time.
     * 
     * @param observer ContactsObserver currently observing native address book changes
     * @throws RuntimeException if a new observer is being registered without having unregistered the previous one
     */
     ...
     * Only supported on 2.X. 
     * The 1.X implementation always returns null.
     * @return An array containing all the Accounts on the device, or null if none exist 
     */
    public abstract Account[] getAccounts();
    
    /**
     * Fetches all the existing Accounts on the device corresponding to the provided Type
     * Only supported on 2.X.
     * The 1.X implementation always returns null.
     * @param type The Type of Account to fetch
     * @return An array containing all the Accounts of the provided Type, or null if none exist 
     */
     ...
     * Only supported on 2.X. 
     * The 1.X implementation does nothing.
     ...
     * In 1.X devices only the null account is supported, 
     * i.e., a non null account always uses null as a return value.
    ...
    /**
     * Adds a contact.
     * Note that the returned ID data will be the same size of ccList plus 
     * one change containing the NAB Contact ID (at the first position).
     * The remaining IDs correspond to the details in the original change list.
     * Null IDs among these remaining IDs are possible when these correspond to unsupported details.
     * On 1.X Devices only a null account parameter is expected.
     * @param account Account to be associated with the added contact (may be null). 
     * @param ccList The Contact data as a {@link ContactChange} array 
     * @return A {@link ContactChange} array with contact's new ID data or null in case of failure.
     */
    public abstract ContactChange[] addContact(Account account, ContactChange[] ccList);
    
    /**
     * Updates an existing contact.
     * The returned ID data will be the same size of the ccList.
     * However, if a null or empty ccList is passed as an argument then null is returned.
     * Null IDs among the ID data are also possible when these correspond to unsupported details.
     * @param ccList The Contact update data as a {@link ContactChange} array
     * @return A {@link ContactChange} array with the contact's new ID data or null
     */
    ...
    /**
     * Checks whether or not a {@link ContactChange} key is supported.
