...
public class UploadServerContacts extends BaseSyncProcessor {
    ...
    private void processNewContactsResp(final Response resp) {
        ServiceStatus status = BaseEngine.genericHandleResponseType("ContactChanges",
                resp.mDataTypes);
        if (status == ServiceStatus.SUCCESS) {
            ...
            if (dupList.size() > 0) {
                LogUtils.logV("UploadServerContacts.processNewContactsResp() "
    ...
    private void processDeletedContactsResp(final Response resp) {
        ServiceStatus status = BaseEngine.genericHandleResponseType("ContactListResponse",
                resp.mDataTypes);
        if (status == ServiceStatus.SUCCESS) {
    ...
    private void processDeletedDetailsResp(final Response resp) {
        ServiceStatus status = BaseEngine.genericHandleResponseType("ContactDetailDeletion",
                resp.mDataTypes);
        if (status == ServiceStatus.SUCCESS) {
    ...
    private void processGroupDeletionsResp(final Response resp) {
        ServiceStatus status = BaseEngine.genericHandleResponseType("StatusMsg", resp.mDataTypes);
        if (status == ServiceStatus.SUCCESS) {
            if (resp.mDataTypes.size() == 0) {
