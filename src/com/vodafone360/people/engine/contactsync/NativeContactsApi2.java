...
public class NativeContactsApi2 extends NativeContactsApi {    
	...
	/**
	 * Regular expression for a date that can be handled 
	 * by the People Client at present.
	 * Matches the following cases:
	 * N-n-n
	 * n-n-N
	 * Where: 
	 * - 'n' corresponds to one or two digits
	 * - 'N' corresponds to two or 4 digits
	 */
	private static final String COMPLETE_DATE_REGEX = 
	    "(?:^\\d{2,4}-\\d{1,2}-\\d{1,2}$|^\\d{1,2}-\\d{1,2}-\\d{2,4}$)";
	
	/**
	 * 'My Contacts' System group where clause
	 */
    private static final String MY_CONTACTS_GROUP_WHERE_CLAUSE = 
        Groups.SYSTEM_ID+"=\"Contacts\"";   
    /**
     * 'My Contacts' System Group Membership where in clause (Multiple 'My Contacts' IDs)
     */
    private static final String MY_CONTACTS_MULTI_GROUP_MEMBERSHIP = 
        Data.MIMETYPE+"=\""+GroupMembership.CONTENT_ITEM_TYPE+"\" AND "+Data.DATA1+" IN (";
    /**
     * Selection where clause for a NULL Account
     */
    private static final String NULL_ACCOUNT_WHERE_CLAUSE = 
        RawContacts.ACCOUNT_NAME +" IS NULL AND "+ RawContacts.ACCOUNT_TYPE +" IS NULL";
    /**
     * Selection where clause for an Organization detail.
     */
    private static final String ORGANIZATION_DETAIL_WHERE_CLAUSE = 
        RawContacts.Data.MIMETYPE +"=\""+Organization.CONTENT_ITEM_TYPE+"\"";
    /**
	...
	static {    
		...
		sFromNabContentTypeToKeyMap.put(
		        StructuredName.CONTENT_ITEM_TYPE, ContactChange.KEY_VCARD_NAME);
		sFromNabContentTypeToKeyMap.put(
		        Nickname.CONTENT_ITEM_TYPE, ContactChange.KEY_VCARD_NICKNAME);
		sFromNabContentTypeToKeyMap.put(
		        Phone.CONTENT_ITEM_TYPE, ContactChange.KEY_VCARD_PHONE);
		sFromNabContentTypeToKeyMap.put(
		        Email.CONTENT_ITEM_TYPE, ContactChange.KEY_VCARD_EMAIL);
		sFromNabContentTypeToKeyMap.put(
		        StructuredPostal.CONTENT_ITEM_TYPE, ContactChange.KEY_VCARD_ADDRESS);
		sFromNabContentTypeToKeyMap.put(
		        Organization.CONTENT_ITEM_TYPE, ContactChange.KEY_VCARD_ORG);
		sFromNabContentTypeToKeyMap.put(
		        Website.CONTENT_ITEM_TYPE, ContactChange.KEY_VCARD_URL);
		sFromNabContentTypeToKeyMap.put(
		        Note.CONTENT_ITEM_TYPE, ContactChange.KEY_VCARD_NOTE);
        sFromNabContentTypeToKeyMap.put(
                Event.CONTENT_ITEM_TYPE, ContactChange.KEY_VCARD_DATE);
//        sFromNabContentTypeToKeyMap.put(
//                Photo.CONTENT_ITEM_TYPE, ContactChange.KEY_PHOTO); 
		...
		sFromKeyToNabContentTypeArray.append(
		        ContactChange.KEY_VCARD_NAME, StructuredName.CONTENT_ITEM_TYPE);
		sFromKeyToNabContentTypeArray.append(
		        ContactChange.KEY_VCARD_NICKNAME, Nickname.CONTENT_ITEM_TYPE);
		sFromKeyToNabContentTypeArray.append(
		        ContactChange.KEY_VCARD_PHONE, Phone.CONTENT_ITEM_TYPE);
		sFromKeyToNabContentTypeArray.append(
		        ContactChange.KEY_VCARD_EMAIL, Email.CONTENT_ITEM_TYPE);
		sFromKeyToNabContentTypeArray.append(
		        ContactChange.KEY_VCARD_ADDRESS, StructuredPostal.CONTENT_ITEM_TYPE);
		sFromKeyToNabContentTypeArray.append(
		        ContactChange.KEY_VCARD_ORG, Organization.CONTENT_ITEM_TYPE);
		// Special case: VCARD_TITLE maps to the same NAB type as 
		sFromKeyToNabContentTypeArray.append(
		        ContactChange.KEY_VCARD_TITLE, Organization.CONTENT_ITEM_TYPE);
		sFromKeyToNabContentTypeArray.append(
		        ContactChange.KEY_VCARD_URL, Website.CONTENT_ITEM_TYPE);
		sFromKeyToNabContentTypeArray.append(
		        ContactChange.KEY_VCARD_NOTE, Note.CONTENT_ITEM_TYPE);
		sFromKeyToNabContentTypeArray.append(
		        ContactChange.KEY_VCARD_DATE, Event.CONTENT_ITEM_TYPE);
//		sFromKeyToNabContentTypeArray.append(
//                ContactChange.KEY_PHOTO, Photo.CONTENT_ITEM_TYPE);
     ...
     * The reason for this being an array is because there may be multiple "My Contacts" groups (Platform Bug!?).
     */
    private long[] mMyContactsGroupRowIds = null;
    /**
     * Arguably another example where Organization and Title force us into extra effort.
     * We use this value to pass the correct detail ID when an 'add detail' is done for one the two
     * although the other is already present.
     ...
     * TODO: Move to own class if batches become supported in other areas
     */
    ...
    /**
     * Convenience interface to map the generic DATA column names to the
     * People profile detail column names.
     */
    private interface PeopleProfileColumns {
        ...
        public static final String CONTENT_ITEM_TYPE = 
            "vnd.android.cursor.item/com.vodafone360.people.profile";
    ...
    /**
     * This class holds a native content observer that will be notified in case of changes
    ...
    public void registerObserver(ContactsObserver observer) {
        ...
        if (mAbstractedObserver != null) {
            throw new RuntimeException("Only one observer at a time supported... Please unregister first.");
        }
    ...
    public Account[] getAccountsByType(String type) {
        ...
        android.accounts.Account[] accounts2xApi = 
            accountMan.getAccountsByType(type);
    ...
    public boolean addPeopleAccount(String username) {
        ...
        try {
            android.accounts.Account account = 
                new android.accounts.Account(username, PEOPLE_ACCOUNT_TYPE);
            AccountManager accountMan = AccountManager.get(mContext);
    ...
    public ContactChange[] addContact(Account account, ContactChange[] ccList) {
        ...
        ContentProviderOperation.Builder builder = ContentProviderOperation.newInsert(
                addCallerIsSyncAdapterParameter(RawContacts.CONTENT_URI)).withYieldAllowed(mYield).withValues(mValues);
        ...
        for(int i = 0 ; i < ccListSize; i++) {
            ...
            if(cc != null) {
                ...
                if(key == ContactChange.KEY_VCARD_ORG && 
                        mMarkedOrganizationIndex < 0) {
                    // Mark for later writing
                    mMarkedOrganizationIndex = i;
                    continue;
                } 
                if(key == ContactChange.KEY_VCARD_TITLE && 
                        mMarkedTitleIndex < 0) {
                    // Mark for later writing
                    mMarkedTitleIndex = i;
                    continue;
        ...
        // Special case for the organization/title if it was flagged in the putDetail call
    ...
    public ContactChange[] updateContact(ContactChange[] ccList) {
        if(ccList == null || ccList.length == 0) {
            LogUtils.logW(
                "NativeContactsApi2.updateContact() nothing to update - empty ccList!");
            return null;
        }
        ...
        if(nabContactId == ContactChange.INVALID_ID) {
            LogUtils.logW(
                    "NativeContactsApi2.updateContact() Ignoring update request because of invalid NAB Contact ID. Internal Contact ID is "+
            ...
            switch(cc.getType()) {
                ...
                case ContactChange.TYPE_UPDATE_DETAIL:
                    if(cc.getNabDetailId() != ContactChange.INVALID_ID) {
                        ...
                        LogUtils.logW("NativeContactsApi2.updateContact() Ignoring update to detail for "+
                                cc.getKeyToString()+
                                " because of invalid NAB Detail ID. Internal Contact is "+
                                cc.getInternalContactId()+
                                ", Internal Detail Id is "+cc.getInternalDetailId());
                    }
                    break;
                case ContactChange.TYPE_DELETE_DETAIL:
                    if(cc.getNabDetailId() != ContactChange.INVALID_ID) {
                        ...
                        LogUtils.logW("NativeContactsApi2.updateContact() Ignoring detail deletion for "+
                                cc.getKeyToString()+
                                " because of invalid NAB Detail ID. Internal Contact is "+
                                cc.getInternalContactId()+
                                ", Internal Detail Id is "+cc.getInternalDetailId());
    ...
    public void removeContact(long nabContactId) {
        mCr.delete(addCallerIsSyncAdapterParameter(
                ContentUris.withAppendedId(RawContacts.CONTENT_URI, nabContactId)), 
                null, null);
    }
     ...
     * Allows a cleaner separation the public API method. 
    ...
    private long[] getContactIds(final String selection) {
            ...
            if(cursorCount > 0) {
                ...
                while(cursor.moveToNext()) {
                    ...
                    final String accountType = 
                        cursor.getString(CONTACTID_PROJECTION_ACCOUNT_TYPE);
                    // TODO: Remove hardcoding (if statement)
                    if(TextUtils.isEmpty(accountType) || 
                            accountType.equals(NativeContactsApi.PEOPLE_ACCOUNT_TYPE) || 
        ...
        if(idCount > 0) {
            // Here we copy the array to strip any eventual nulls at the end of the tempIds array
    ...
    /**
     * Fetches the IDs corresponding to the "My Contacts" System Group
     * The reason why we return an array is because there may be multiple "My Contacts" groups.
     * @return Array containing all IDs of the "My Contacts" group or null if none exist
     */
    ...
    private boolean isContactInMyContactsGroup(long nabContactId) {
        ...
        if(mMyContactsGroupRowIds != null) {            
            final Uri dataUri = 
                Uri.withAppendedPath(ContentUris.withAppendedId(RawContacts.CONTENT_URI, nabContactId),
				...
				default:
					...
					if(!TextUtils.isEmpty(value)) {
					    ...
					    final ContactChange cc = new ContactChange(
					            key, value, ContactChange.FLAG_NONE); 
	...
	private void readName(Cursor cursor, 
	        List<ContactChange> ccList, long nabContactId) {
		// Using display name only to check if there is a valid name to read
		final String displayName = 
		    CursorUtils.getString(cursor, StructuredName.DISPLAY_NAME);
		if(!TextUtils.isEmpty(displayName)) {
		    final long nabDetailId = 
		        CursorUtils.getLong(cursor, StructuredName._ID);
			// VCard Helper data type (CAB)
			final Name name = new Name();
			// NAB: Given name -> CAB: First name
			name.firstname = 
			    CursorUtils.getString(cursor, StructuredName.GIVEN_NAME);
			// NAB: Family name -> CAB: Surname
			name.surname = 
			    CursorUtils.getString(cursor, StructuredName.FAMILY_NAME);
			// NAB: Prefix -> CAB: Title
			name.title = 
			    CursorUtils.getString(cursor, StructuredName.PREFIX);
			// NAB: Middle name -> CAB: Middle name
			name.midname = 
			    CursorUtils.getString(cursor, StructuredName.MIDDLE_NAME);
			// NAB: Suffix -> CAB: Suffixes
			name.suffixes = 
			    CursorUtils.getString(cursor, StructuredName.SUFFIX);
	...
	/**
	 * Reads an nickname detail as a {@link ContactChange} from the provided cursor.
	 * 
	 * @param cursor Cursor to read from
     * @param ccList List of Contact Changes to add read detail data
     * @param nabContactId ID of the NAB Contact
	...
	private void readNickname(Cursor cursor, 
		...
		if(!TextUtils.isEmpty(value)) {
			...
			/* 
			 * TODO: Decide what to do with nickname:
			 * Can only have one in VF360 but NAB Allows more than one!
			 */
			final ContactChange cc = new ContactChange(
			        ContactChange.KEY_VCARD_NICKNAME, 
			        value, ContactChange.FLAG_NONE);
			cc.setNabContactId(nabContactId);
			cc.setNabDetailId(nabDetailId);
			ccList.add(cc);
		}
	...
	/**
	 * Reads an phone detail as a {@link ContactChange} from the provided cursor.
	 * 
	 * @param cursor Cursor to read from
     * @param ccList List of Contact Changes to add read detail data
     * @param nabContactId ID of the NAB Contact
	...
	private void readPhone(Cursor cursor, 
		...
		if(!TextUtils.isEmpty(value)) {
			...
			final boolean isPrimary = 
			    CursorUtils.getInt(cursor, Phone.IS_PRIMARY) != 0;
	...
	/**
	 * Reads an email detail as a {@link ContactChange} from the provided cursor.
	 * 
	 * @param cursor Cursor to read from
     * @param ccList List of Contact Changes to add read detail data
     * @param nabContactId ID of the NAB Contact
	...
	private void readEmail(Cursor cursor, 
		...
		if(!TextUtils.isEmpty(value)) {
			...
			final boolean isPrimary = 
			    CursorUtils.getInt(cursor, Email.IS_PRIMARY) != 0;
	...
	/**
	 * Reads an address detail as a {@link ContactChange} from the provided cursor.
	 * For this type of detail we need to use a VCARD (semicolon separated) value.
	 * 
	 * @param cursor Cursor to read from
     * @param ccList List of Contact Changes to add read detail data
	...
	private void readAddress(Cursor cursor, 
	        List<ContactChange> ccList, long nabContactId) {
		// Using formatted address only to check if there is a valid address to read
		final String formattedAddress = 
		    CursorUtils.getString(cursor, StructuredPostal.FORMATTED_ADDRESS);
		if(!TextUtils.isEmpty(formattedAddress)) {
			...
			final boolean isPrimary = 
			    CursorUtils.getInt(cursor, StructuredPostal.IS_PRIMARY) != 0;
			...
			address.addressLine1 = 
			    CursorUtils.getString(cursor, StructuredPostal.STREET);
			// NAB: PO Box -> CAB: postOfficeBox
			address.postOfficeBox = 
			    CursorUtils.getString(cursor, StructuredPostal.POBOX);
			// NAB: Neighborhood -> CAB: AddressLine2
			address.addressLine2 =
			    CursorUtils.getString(cursor, StructuredPostal.NEIGHBORHOOD);
			// NAB: City -> CAB: City
			address.city = 
			    CursorUtils.getString(cursor, StructuredPostal.CITY);
			// NAB: Region -> CAB: County
			address.county = 
			    CursorUtils.getString(cursor, StructuredPostal.REGION);
			// NAB: Post code -> CAB: Post code
			address.postCode = 
			    CursorUtils.getString(cursor, StructuredPostal.POSTCODE);
			// NAB: Country -> CAB: Country
			address.country = 
			    CursorUtils.getString(cursor, StructuredPostal.COUNTRY);
			
			final ContactChange cc = new ContactChange(
			        ContactChange.KEY_VCARD_ADDRESS, 
					VCardHelper.makePostalAddress(address), 
					flags);
			cc.setNabContactId(nabContactId);
			cc.setNabDetailId(nabDetailId);
			ccList.add(cc);
	...
	/**
	 * Reads an organization detail as a {@link ContactChange} from the provided cursor.
	 * For this type of detail we need to use a VCARD (semicolon separated) value.
	 * 
	...
	private void readOrganization(Cursor cursor, 
		...
		final boolean isPrimary = 
		    CursorUtils.getInt(cursor, Organization.IS_PRIMARY) != 0;
	...
	/**
     * Reads an Website detail as a {@link ContactChange} from the provided cursor.
     * 
     * @param cursor Cursor to read from
     * @param ccList List of Contact Changes to add read detail data
     * @param nabContactId ID of the NAB Contact
	...
	private void readWebsite(Cursor cursor, 
		...
		if(!TextUtils.isEmpty(url)) {
			...
			final boolean isPrimary = 
			    CursorUtils.getInt(cursor, Website.IS_PRIMARY) != 0;
	...
	/**
     * Reads an Birthday detail as a {@link ContactChange} from the provided cursor.
	...
	public void readBirthday(Cursor cursor,
	    ...
	    if(type == Event.TYPE_BIRTHDAY) {
	        ...
	        // FIXME: Remove this check when/if the backend becomes able to handle incomplete birthdays
	        if(date != null && date.matches(COMPLETE_DATE_REGEX)) {
    	        ...
    	        final ContactChange cc = new ContactChange(
    	                ...
    	                date, ContactChange.FLAG_BIRTHDAY);
    	        cc.setNabContactId(nabContactId);
                cc.setNabDetailId(nabDetailId);
                ccList.add(cc);
	 ...
	 * @param nabContactId The existing NAB Contact ID if it is an update or an invalid id if a new contact
	 */
	private void addValuesToBatch(long nabContactId) {
        ...
        if(mValues.size() > 0) {
            ...
            ContentProviderOperation.Builder builder = ContentProviderOperation.newInsert(
                    addCallerIsSyncAdapterParameter(Data.CONTENT_URI)).withYieldAllowed(mYield).withValues(mValues);
	...
	private void addUpdateValuesToBatch(long nabDetailId) {
        if(mValues.size() > 0) {
            ...
            ContentProviderOperation.Builder builder = ContentProviderOperation.newUpdate(
                    addCallerIsSyncAdapterParameter(uri)).withYieldAllowed(mYield).withValues(mValues);
            mYield = false;
            mBatch.add(builder.build());
        }	    
	}
	...
	private void addProfileAction(long internalContactId) {
        ...
        mValues.put(PeopleProfileColumns.DATA_SUMMARY, 
                mContext.getString(R.string.android_contact_profile_summary));
        mValues.put(PeopleProfileColumns.DATA_DETAIL, 
...
//	}
	...
	private void putBirthday(ContactChange cc) {
	    if((cc.getFlags() & ContactChange.FLAG_BIRTHDAY) == 
...
//	}
        ...
        if(mMarkedOrganizationIndex > -1) {
            ...
            if(organization != null) {
                ...
                if(organization.unitNames.size() > 0) {
                    // Only considering one unit name (department) as that's all we support
     ...
     * The end of result of this is that the Organization may be inserted, updated or deleted
     * depending on the update data. 
     * For example, if the title is deleted but there is also a company name then the Organization is just updated.
     * However, if there was no company name then the detail should be deleted altogether.
     * @param ccList {@link ContactChange} list where Organization and Title may be found
    ...
    private void updateOrganization(ContactChange[] ccList, long nabContactId) {
        ...
        Cursor cursor = mCr.query(
                uri, null, ORGANIZATION_DETAIL_WHERE_CLAUSE, null, RawContacts.Data._ID);
        String company = null;
        ...
        if(company != null || department != null || title != null) {
             ...
             * we assume a insert or update is needed.
             */
            mValues.clear();
            mValues.put(Organization.LABEL, (String) null);
            ...
            /* Had an Organization but now all values are null,
             * delete is in order.
             */
     ...
     * @return {@link ContactChange} array containing new IDs (may contain some null elements)
     */
    private ContactChange[] executeNewContactBatch(ContactChange[] ccList) {
        ...
        while(resultsIndex < resultsSize) { 
            if(ccListIndex == mMarkedOrganizationIndex || 
                    ccListIndex == mMarkedTitleIndex) {
                ccListIndex++;
                continue;
            }
            ...
            if(resultsIndex == resultsSize - 1 && haveOrganization) {
              // for readability we leave Organization/Title for outside the loop
              break;
            }
            ...
            if(sFromKeyToNabContentTypeArray.indexOfKey(cc.getKey()) >= 0) {
                final long nabDetailId = 
                    ContentUris.parseId(results[resultsIndex].uri);
                final ContactChange idChange = 
                    ContactChange.createIdsChange(ccList[ccListIndex],
        ...
        if(haveOrganization) {
            ...
            if(mMarkedOrganizationIndex > -1) {
                final ContactChange idChange = 
                    ContactChange.createIdsChange(
                        ccList[mMarkedOrganizationIndex], 
                        ContactChange.TYPE_UPDATE_NAB_DETAIL_ID);
                idChange.setNabContactId(nabContactId);
                idChange.setNabDetailId(nabDetailId);                
        ...
        if(haveOrganization) {
            ...
            if(mExistingOrganizationId != nabDetailId) {
                ...
                throw new RuntimeException("NativeContactsApi2.executeUpdateContactBatch()"
                        + "Unexpected null Organization URI for NAB Contact:"+ccList[0].getNabContactId());
            }
    ...
    /**
     * Utility method used to create an entry in the 
     * ContactsContract.Settings database table for the 360 People Account.
     * This method only exists to compensate for HTC devices 
     * such as Legend or Desire that don't create the entry automatically
     * like the Nexus One.
    ...
    private void createSettingsEntryForAccount(String username) {
        ...
        mValues.put(Settings.ACCOUNT_TYPE, PEOPLE_ACCOUNT_TYPE);
    ...
    /**
     * Maps a phone type from the native value to the {@link ContactChange} flags.
	...
	private static int mapToNabPhoneType(int flags) { 
	    if((flags & ContactChange.FLAGS_WORK_CELL) == 
	        ContactChange.FLAGS_WORK_CELL) {
	        return Phone.TYPE_WORK_MOBILE;
	   }
	    
       if((flags & ContactChange.FLAGS_HOME_FAX) == 
            ContactChange.FLAGS_HOME_FAX) {
            return Phone.TYPE_FAX_HOME; 
       }
	    
       if((flags & ContactChange.FLAGS_WORK_FAX) == 
            ContactChange.FLAGS_WORK_FAX) {
            return Phone.TYPE_FAX_WORK; 
       }
       
       if((flags & ContactChange.FLAG_HOME) ==
           ContactChange.FLAG_HOME) {
           return Phone.TYPE_HOME;
       }
       
       if((flags & ContactChange.FLAG_WORK) == 
           ContactChange.FLAG_WORK) {
           return Phone.TYPE_WORK;
       }
       
       if((flags & ContactChange.FLAG_CELL) == 
           ContactChange.FLAG_CELL) {
           return Phone.TYPE_MOBILE;
       }
       
       if((flags & ContactChange.FLAG_FAX) == 
           ContactChange.FLAG_FAX) {
    ...
    /**
     * Maps a email type from the native value into the {@link ContactChange} flags
    ...
    /**
     * Maps a address type from the native value into the {@link ContactChange} flags
     * @param nabType Native email type
     * @return {@link ContactChange} flags
     */
    ...
    /**
     * Maps a organization type from the native value into the {@link ContactChange} flags
    ...
    /**
     * Maps a Website type from the native value into the {@link ContactChange} flags
     * @param nabType Native email type
     * @return {@link ContactChange} flags
     */
