...
//                ContactChange.KEY_PHOTO, Photo.CONTENT_ITEM_TYPE);
    ...
    public Account getVendorSpecificAccount() {
        ...
        Cursor c = mContext.getContentResolver().query(Settings.CONTENT_URI, PROJECTION, null,
                null, null);
        String[] values = new String[c.getCount()];
        for (int i = 0; i < values.length; i++) {
            c.moveToNext();

            if (isVendorSpecificAccount(c.getString(1))) {
                return new Account(c.getString(0), c.getString(1));
            }
        }
    ...
    public boolean addPeopleAccount(String username) {
        ...
        if (isAdded) {
            ...
            // done one time
            // just before first time sync. In the future, this code may change
            // if we modify
